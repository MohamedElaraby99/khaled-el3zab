import rechargeCosahlnahadel from "../models/rechargeCode.model.js";
...
const generateRechargeCode = async (req, res, next) => {
    try {
        ...
        for (let i = 0; i < quantity; i++) {
            ...
            while (!isUnique) {
                code = rechargeCosahlnahadel.generateCode();
                const existingCode = await rechargeCosahlnahadel.findOne({ code });
                if (!existingCode) {
                    isUnique = true;
                }
            }

            const rechargeCode = await rechargeCosahlnahadel.create({
                code,
...
const getAllRechargeCodes = async (req, res, next) => {
    try {
        ...
        const codes = await rechargeCosahlnahadel.find(query)
        ...
        const total = await rechargeCosahlnahadel.countDocuments(query);
        const totalPages = Math.ceil(total / limit);

        // Calculate statistics
        const stats = await rechargeCosahlnahadel.aggregate([
            {
                $group: {
                    _id: null,
                    totalCodes: { $sum: 1 },
...
const deleteRechargeCode = async (req, res, next) => {
    try {
        ...
        const code = await rechargeCosahlnahadel.findById(id);
        if (!code) {
        ...
        await rechargeCosahlnahadel.findByIdAndDelete(id);
...
const getRechargeCodeStats = async (req, res, next) => {
    try {
        ...
        // Get recent codes
        const recentCodes = await rechargeCosahlnahadel.find()
            .sort({ createdAt: -1 })
            .limit(5)
            .populate('createdBy', 'fullName');

        // Get monthly stats
        const monthlyStats = await rechargeCosahlnahadel.aggregate([
            {
                $group: {
                    _id: {
                        year: { $year: '$createdAt' },
